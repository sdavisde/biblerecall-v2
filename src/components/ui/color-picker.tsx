...
export function ColorPicker({ color = 'hsl(0 0% 0%)', onChange }: ColorPickerProps) {
  const [currentColor, setCurrentColor] = React.useState(color)
  const [colorMode, setColorMode] = React.useState<ColorMode>('hsl')
  ...
  const hsl = currentColor.startsWith('hsl')
    ? ColorUtils.parseHslString(color) ?? { h: 0, s: 0, l: 0 }
    : ColorUtils.rgbToHsl(ColorUtils.hexToRgb(color) ?? { r: 0, g: 0, b: 0 })
  const rgb = ColorUtils.hslToRgb(hsl) || { r: 0, g: 0, b: 0 }
  const rgbaString = ColorUtils.formatRgba(rgb)

  const handleColorChange = (newColor: string) => {
    setCurrentColor(newColor)
    onChange?.(ColorUtils.parseHslString(newColor))
  }

  const updateHSL = (h: number, s: number, l: number) => {
    const newHslString = `hsl(${h} ${s}% ${l}%)`
    handleColorChange(newHslString)
  }
  ...
  const handleHexChange = (hex: string) => {
    if (/^#[0-9A-Fa-f]{6}$/.test(hex)) {
      ...
      if (rgb) {
        ...
        handleColorChange(ColorUtils.formatHsl(hsl))
      }
    }
  }
  ...
  const handleRgbChange = (key: keyof typeof rgb, value: string) => {
    ...
    if (!isNaN(numValue) && numValue >= 0 && numValue <= 255) {
      ...
      const newHsl = ColorUtils.rgbToHsl(newRgb)
      handleColorChange(ColorUtils.formatHsl(newHsl))
    }
  }
