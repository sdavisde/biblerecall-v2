...
function MenuItem({ item, index, toggleMenu }: { item: NavItem; index: number; toggleMenu: () => void }) {
  ...
  const buttonContent = (
    ...
    >
      ...
      <div
        className={cn('flex h-10 w-10 items-center justify-center rounded-full  text-secondary-foreground shadow-md', {
          'bg-secondary': !Lodash.isNil(item.href),
          'bg-muted': Lodash.isNil(item.href), // Using this to determine "add" button
        })}
      >
        <item.icon size={20} />
      </div>
